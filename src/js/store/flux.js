		...
		actions: {
			...
			getContacts: async () => {
				try {
					...
					const response = await fetch(`https://playground.4geeks.com/contact/agendas/${currentAgendaSlug}`);
			...
			addContact: async (newContact) => {
				try {
					...
					setStore({ agenda: [...getStore().agenda, data] });
					await getActions().getContacts(); // Fetch the updated contact list
